...
file_psd_props = "../data/PSD_properties_application.csv"
...
filter_miss = np.logical_not(filter_q)
print("Number of samples with quality OK:", np.sum(filter_q.values))
print("Number of samples with quality NOT OK:", np.sum(filter_miss.values))
print(len(filter_q.values))
### filtering all data in agreement with K measurement quality check
data = data[filter_miss]
# data = data[filter_q]
...
### drop samples with NAN values in Kf and sieve samples and reset index in data frame
data_app.dropna(inplace = True)
data_app.reset_index(drop=True,inplace=True)
print(len(data_app.index))
data_app.to_csv(file_application_data)
